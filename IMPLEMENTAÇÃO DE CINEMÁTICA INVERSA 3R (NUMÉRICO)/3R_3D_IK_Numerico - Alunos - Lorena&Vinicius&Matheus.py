...
############################ CINEMÁTICA INVERSA ######################################
######################################################################################
def cinematica_inv_num_Ji(xd, yd, zd, q0_m, q1_m, q2_m, dt):
    ...
    if r >= a0-a1-a2 and r <= a0+a1+a2: # Workspace
        #Inicializa vetor Cinemática Direta
        Cd = np.array(( ((a1*np.sin(q[1])+a2*np.cos(q[1]+q[2]))*np.cos(q[0]) ) ,  
                        ((a1*np.sin(q[1])+a2*np.cos(q[1]+q[2]))*np.sin(q[0]) ) ,
                        (a0+a1*np.cos(q[1])-a2*np.sin(q[1]+q[2]) ) 
                     ))
        ...
        J = np.array(( ( -(a1*np.sin(q[1])+a2*np.cos(q[1]+q[2]))*np.sin(q[0]) , (a1*np.cos(q[1])-a2*np.sin(q[1]+q[2]))*np.cos(q[0]), -a2*np.sin(q[1]+q[2])*np.cos(q[0])  ), 
                       ( (a1*np.sin(q[1])+a2*np.cos(q[1]+q[2]))*np.cos(q[0])  , (a1*np.cos(q[1])-a2*np.sin(q[1]+q[2]))*np.sin(q[0]), -a2*np.sin(q[0])*np.sin(q[1]+q[2])  ),
                       ( 0                                                    , -a1*np.sin(q[1])-a2*np.cos(q[1]+q[2])              , -a2*np.cos(q[1]+q[2]) ),
...
def cinematica_inv_num_Jt(xd, yd, zd, q0_m, q1_m, q2_m, dt):
    a0 = 0.3
    a1 = 0.5
    a2 = 0.5
